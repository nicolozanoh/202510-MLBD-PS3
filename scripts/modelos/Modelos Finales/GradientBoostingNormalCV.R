...
# Definir la receta para el preprocesamiento de datos
receta <- recipe(price ~ srf_ttl+surf_cv+rooms+bdrms+bathrm+prp_typ+PC1+PC2+
  ...
  step_novel(all_nominal_predictors()) %>%  # Manejar nuevos niveles en factores
  step_dummy(all_nominal_predictors()) %>%  # Convertir categóricas a dummies
  step_interact(terms = ~ srf_ttl:matches("estrato")+dst_vpr:matches("estrato")) %>% # Interacciones
  step_poly(dst_vpr, degree = 2) %>% # Término polinómico
  step_zv(all_predictors()) %>%  # Eliminar predictores con varianza cero
  step_normalize(all_predictors()) # Normalizar predictores
...
# Aplicar la receta a los conjuntos de datos
...
write.csv(results, "stores\\sub\\gbm_no_espacial.csv", row.names = FALSE)
