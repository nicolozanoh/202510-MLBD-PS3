...
mae_results <- train_nn(
  nn_3l, "5 HL- 4096 neur, 64 batch, adam", 500, 128, 30, "adam", mae_results
)

pred_baked <- bake(receta2, new_data = pred_df, all_predictors())

x_pred <- pred_baked %>% as.matrix()
predictions <- nn_3l %>% predict(x_pred)

results <- bind_cols(pred_df$prpty_, predictions)

colnames(results) <- c("property_id","price")

write.csv(results, "stores\\sub\\nn_3layers.csv", row.names = FALSE)